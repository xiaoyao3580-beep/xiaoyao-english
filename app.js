...
function computeReports() {
  ...
  const practiceRecords = rawPracticeReportRecords().filter(record => inReportScope(record) && (cfg.selectedLevelId === 'all' || record.levelId === cfg.selectedLevelId || !record.levelId) && (cfg.selectedLessonId === 'all' || record.lessonId === cfg.selectedLessonId));
  ...
  return { records, latestRows, summaryRows, comparisonOptions, selected, keys, chart, buckets, processReports, stats:{ studentCount, classCount, submitCount:latestRows.length, averageScore:reportAverage(latestRows.map(r => r.scorePercent)) }, lessons:reportLessonOptions(cfg, rawRecords) };
}
