...
const CUSTOM_ONLINE_LEVEL_ID = 'custom-online-regular';
const CUSTOM_ONLINE_LEVEL_TITLE = '定制化线上常规课';
...
const HOME_META = {
  ...
  'custom-online-regular':['Custom Online','assignment','bg-[#e7e4ff]','text-[#5827fc]']
};
const LEVEL_META = {
  ...
  'custom-online-regular':['Custom Online Track','assignment','from-[#5827fc] via-[#7252ff] to-[#b3a7ff]','bg-[#e7e4ff]','text-[#5827fc]']
};
const SLIDES = [
...
function isStudentClassCode(code) { const normalized = String(normalizeClass(code) || '').toLowerCase(); return state.students.some(s => String(s.id).toLowerCase() === normalized); }
function studentByClassCode(code) { const normalized = String(normalizeClass(code) || '').toLowerCase(); return state.students.find(s => String(s.id).toLowerCase() === normalized) || null; }
function studentHasCustomHomework(u) { return !!(u && state.homework.some(h => String(normalizeClass(h.classCode)).toLowerCase() === String(u.id).toLowerCase())); }
function customOnlineLevel() { return { id:CUSTOM_ONLINE_LEVEL_ID, title:CUSTOM_ONLINE_LEVEL_TITLE }; }
function homeworkMatchesLevel(h, level, u, isTeacher) {
  if (level.id === CUSTOM_ONLINE_LEVEL_ID) {
    if (isTeacher) return isStudentClassCode(h.classCode);
    return !!(u && String(normalizeClass(h.classCode)).toLowerCase() === String(u.id).toLowerCase());
  }
  return normalizeClass(h.classCode) === level.id;
}
function homeworkClassSelectOptions(selectedCode) {
  const selected = selectedCode ? normalizeClass(selectedCode) : '';
  const studentOptions = state.students.map(s => '<option value="' + esc(s.id.toLowerCase()) + '" ' + (selected === s.id.toLowerCase() ? 'selected' : '') + '>' + esc(s.name || s.id) + '</option>').join('');
  const classOptions = teacherClasses().map(c => '<option value="' + esc(c.code) + '" ' + (selected === c.code ? 'selected' : '') + '>' + esc(c.name) + '</option>').join('');
  return classOptions + '<optgroup label="' + esc(CUSTOM_ONLINE_LEVEL_TITLE) + '">' + studentOptions + '</optgroup>';
}
...
function hasAccess(level) { const u = currentUser(); if (!u) return false; if (u.role === 'teacher') return true; return (level.id === CUSTOM_ONLINE_LEVEL_ID && studentHasCustomHomework(u)) || new Set(classesOf(u)).has(level.id); }
...
function levelPage() {
  let level = levelById(state.level);
  const u = currentUser();
  if (!level && state.level === CUSTOM_ONLINE_LEVEL_ID) {
    level = customOnlineLevel();
  }
  ...
  const uploadedItems = state.homework
    .filter(h => homeworkMatchesLevel(h, level, u, isTeacher) && h.file && h.status !== 'deleted' && !registryIds.has(h.id))
...
function homeworkAdminUnitSections(classHw) {
  ...
  return units.map(u => {
    ...
    const items = unitItems.map(homeworkAdminItemRow).join('');
...
function customHomeworkAdminSection(rows) {
  const customRows = rows.filter(h => isStudentClassCode(h.classCode));
  if (!customRows.length) return '';
  const byStudent = new Map();
  customRows.forEach(h => {
    const key = String(normalizeClass(h.classCode)).toLowerCase();
    if (!byStudent.has(key)) byStudent.set(key, []);
    byStudent.get(key).push(h);
  });
  const inner = Array.from(byStudent.entries()).map(([studentId, classHw]) => {
    const student = studentByClassCode(studentId);
    const label = (student && student.name) || studentId.toUpperCase();
    return '<div class="mb-4 overflow-hidden rounded-[1.25rem] border border-[#F4F2FF] bg-white shadow-sm"><div class="flex items-center justify-between gap-4 bg-[#F8F8FC] px-5 py-3"><span class="min-w-0"><span class="block truncate text-[13px] font-extrabold text-[#2D2A4A] md:text-[14px]">' + esc(label) + '</span><span class="mt-0.5 block text-[11px] font-bold text-[#6B48FF]/70">' + classHw.length + ' 个作业</span></span></div><div class="px-2 pb-2">' + homeworkAdminUnitSections(classHw) + '</div></div>';
  }).join('');
  const classOpenCount = customRows.filter(h => h.status === 'open').length;
  return '<details class="group mb-4 overflow-hidden rounded-[1.35rem] border border-gray-100 bg-white shadow-sm"><summary class="flex min-h-[64px] cursor-pointer list-none items-center justify-between gap-4 bg-[#F4F2FF] px-5 py-4"><span class="flex min-w-0 items-center gap-3"><span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-[#6B48FF] shadow-sm"><i class="fa-solid fa-user-graduate text-sm"></i></span><span class="min-w-0"><span class="block truncate text-sm font-extrabold text-[#2D2A4A] md:text-base">' + esc(CUSTOM_ONLINE_LEVEL_TITLE) + '</span><span class="mt-0.5 block text-xs font-bold text-[#6B48FF]/70">' + customRows.length + ' 个作业 · ' + classOpenCount + ' 个开放 · ' + byStudent.size + ' 名学生</span></span></span><i class="fa-solid fa-chevron-down shrink-0 text-xs text-[#6B48FF] transition-transform group-open:rotate-180"></i></summary><div class="bg-white p-3 pb-1">' + inner + '</div></details>';
}
function homeworkUploadPanel() {
  ...
  const byClass = teacherClasses().map(cls => {
    ...
    const unitHtml = homeworkAdminUnitSections(classHw);
  ...
  }).join('') + customHomeworkAdminSection(rows);
  const classOptions = homeworkClassSelectOptions();
...
function showHomeworkEditor(id) {
  ...
  const classOptions = homeworkClassSelectOptions(currentClass);
