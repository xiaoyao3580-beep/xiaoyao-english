...
function oneToOneReportsPanel() {
  ...
  return '<div class="tab-content active -mx-2 space-y-5 md:-mx-6 lg:-mx-10">' +
    '<section class="overflow-hidden rounded-[2rem] bg-[linear-gradient(135deg,#1d4ed8_0%,#2563eb_38%,#38bdf8_72%,#bfdbfe_100%)] p-6 text-white shadow-[0_24px_70px_-34px_rgba(37,99,235,0.65)] md:p-8 xl:p-10">' +
      '<div class="grid gap-6 xl:grid-cols-[minmax(0,0.92fr)_minmax(360px,0.58fr)] xl:items-stretch"><div class="flex min-h-[18rem] flex-col justify-between"><div><p class="text-xs font-black uppercase tracking-[0.3em] text-white/76">Personal Lesson Report</p><h2 class="mt-3 max-w-3xl text-4xl font-black tracking-tight md:text-6xl">一对一全景诊断</h2><p class="mt-5 max-w-3xl text-sm font-bold leading-7 text-white/86 md:text-base">这里单独查看一对一学生的每一次真实提交、错题波动、用时和详细过程。班课学生继续使用原来的报表。</p></div><div class="mt-6 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end"><label class="block rounded-[1.4rem] bg-white/16 p-4 backdrop-blur-md"><span class="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-white/76">学生</span><select data-report-field="selectedOneStudentId" class="min-h-[48px] w-full rounded-2xl border border-white/20 bg-white px-4 text-sm font-black text-[#2D2A4A] outline-none">' + studentOptions + '</select></label><button data-report-refresh class="inline-flex min-h-[48px] items-center justify-center gap-2 rounded-full bg-white px-5 text-sm font-black text-[#2563eb] shadow-lg shadow-blue-900/10 active-scale">' + (cfg.loading ? '<i class="fa-solid fa-spinner fa-spin"></i>刷新中' : '<i class="fa-solid fa-rotate"></i>刷新数据') + '</button></div></div>' +
      oneReportDiagnosisPanel(records, issues, modules) + '</div>' +
    '</section>' +
    '<section class="grid gap-4 px-2 md:px-6 xl:grid-cols-[22rem_minmax(0,1fr)]">' +
      ...
      '<main class="space-y-4">' +
        '<section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">' + oneReportLatestAccuracyCard(latest, issues) + oneReportStatCard('平均正确率', records.length ? avg + '%' : '--', 'fa-chart-line') + oneReportStatCard('最高正确率', records.length ? best + '%' : '--', 'fa-trophy', 'green') + oneReportStatCard('首末变化', records.length > 1 ? (delta >= 0 ? '+' : '') + delta + '%' : '--', 'fa-arrow-trend-up', delta >= 0 ? 'green' : 'red') + '</section>' +
        '<section class="card-solid overflow-hidden"><div class="flex flex-col gap-3 border-b border-gray-100 px-5 py-5 xl:flex-row xl:items-center xl:justify-between"><div><p class="text-xs font-black uppercase tracking-[0.24em] text-gray-400">Module Overview</p><h3 class="mt-2 text-xl font-black text-[#2D2A4A]">练习模块概览</h3></div><span class="rounded-full bg-[#eff6ff] px-3 py-1 text-xs font-black text-[#2563eb]">点开每条看批次与知识点</span></div><div class="space-y-3 px-4 py-4 md:px-6">' + oneReportModuleCards(modules) + '</div></section>' +
