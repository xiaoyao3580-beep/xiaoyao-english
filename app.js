...
function hasAccess(level) { const u = currentUser(); if (!u || u.role === 'teacher') return true; return new Set(classesOf(u)).has(level.id); }
function card(level, i) { const meta = HOME_META[level.id] || HOME_META.a1; const u = currentUser(); const locked = u && u.role === 'student' && !hasAccess(level); const btn = locked ? 'bg-[#eef1f8] text-[#74777c] ring-1 ring-inset ring-[#d7dbea] shadow-[inset_0_1px_0_rgba(255,255,255,0.95)]' : 'bg-[#5827fc] text-white shadow-[0_8px_20px_-4px_rgba(88,39,252,0.4)]'; const size = level.title.length > 18 ? 'text-base leading-snug' : 'text-lg'; return '<button class="motion-list-item motion-card relative flex min-h-[12.75rem] flex-col items-center rounded-[1.75rem] bg-white p-5 text-center shadow-[0_12px_40px_-5px_rgba(92,45,255,0.08)] md:min-h-[14.5rem] md:rounded-[2rem] md:p-6" style="--motion-delay:' + (i*55) + 'ms" ' + (locked ? 'data-toast="这个课程还没有开放给你"' : 'data-route="level" data-level="' + esc(level.id) + '"') + '><div class="motion-icon flex h-16 w-16 items-center justify-center rounded-full ' + meta[2] + ' shadow-[inset_0_-4px_8px_rgba(0,0,0,0.08)] md:h-20 md:w-20"><span class="material-symbols-outlined text-4xl ' + meta[3] + '">' + meta[1] + '</span></div><div class="mt-4 flex flex-1 flex-col items-center justify-center"><h3 class="whitespace-pre-line font-bold text-[#2c2f33] ' + size + '">' + esc(level.title) + '</h3><p class="mt-1 text-[10px] font-bold uppercase tracking-[0.24em] text-[#595b61]">' + esc(meta[0]) + '</p></div><span class="mt-4 inline-flex min-h-[40px] w-full items-center justify-center rounded-full px-4 py-2.5 text-center text-[12px] font-semibold tracking-[0.01em] ' + btn + '">' + (locked ? '<span class="material-symbols-outlined mr-1 text-[15px] leading-none">lock</span>Locked' : 'Enter') + '</span></button>'; }
...
function openLesson(id, file) { location.href = lessonHref(file, id); }
